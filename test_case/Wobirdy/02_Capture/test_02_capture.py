...
@allure.epic("02_Capture")
@allure.feature("02_Capture")
class Test_02_capture:
    ...
    def test_bird_ai_by_image_and_save_capture(self, in_data, case_skip):
    ...
    def test_bird_ai_capture_by_image(self, in_data, case_skip):
        INFO.logger.info("data: %s", in_data)
        res = RequestControl(in_data).http_request()
        TearDownHandler(res).teardown_handle()
        Assert(in_data["assert_data"]).assert_equality(
    ...
    def test_capture_favorites(self, in_data, case_skip):
        INFO.logger.info("data: %s", in_data)
        res = RequestControl(in_data).http_request()
        TearDownHandler(res).teardown_handle()
        Assert(in_data["assert_data"]).assert_equality(
    ...
    def test_delete_bird_capture_image(self, in_data, case_skip):
        INFO.logger.info("data: %s", in_data)
        res = RequestControl(in_data).http_request()
        TearDownHandler(res).teardown_handle()
        Assert(in_data["assert_data"]).assert_equality(
    ...
    def test_query_bird_capture_wiki(self, in_data, case_skip):
        INFO.logger.info("data: %s", in_data)
        res = RequestControl(in_data).http_request()
        TearDownHandler(res).teardown_handle()
        Assert(in_data["assert_data"]).assert_equality(
...
if __name__ == '__main__':
    pytest.main(['d:\PytestAutoApi\test_case\Wobirdy\02_Capture\test_02_capture.py', '-s', '-W', 'ignore:Module already imported:pytest.PytestWarning'])
