...
from utils.read_files_tool.regular_control import cache_regular, sql_regular, regular
...
class TearDownHandler:
    ...
    def jsonpath_replace_data(cls, replace_key: Text, replace_value: Dict) -> Text:
        ...
        _new_data = jsonpath_replace(
            change_data=_change_data, key_name="_teardown_case", data_switch=False
        )
    ...
    def dependent_type_response(
        self, teardown_case_data: "SendRequest", resp_data: Dict
    ) -> Text:
        """
        判断依赖类型为当前执行用例的响应数据
        :param teardown_case_data: 后置请求数据
        :param resp_data: 当前执行用例的响应数据
        :return: 处理后的数据
        """
        ...
        if _response_dependent is not False:
            ...
            data = self.jsonpath_replace_data(
                replace_key=_replace_key, replace_value=_resp_case_data
            )
        else:
    ...
    def dependent_type_request(
        self, teardown_case_data: Dict, request_data: Dict
    ) -> None:
        """
        判断依赖类型为请求内容
        :param : teardown_case_data: 后置请求数据
        ...
        """
        try:
            _request_set_value = teardown_case_data.get("set_cache")
            _request_dependent = jsonpath(
                obj=request_data, expr=teardown_case_data["jsonpath"]
            )
            if _request_dependent is not False:
                _request_case_data = _request_dependent[0]
                self.get_cache_name(
                    replace_key=_request_set_value, resp_case_data=_request_case_data
                )
            else:
                raise JsonpathExtractionFailed(
            ...
            raise ValueNotFoundError(
                "teardown中缺少set_value参数,请检查用例是否正确"
            ) from e

    def dependent_self_response(
        self, teardown_case_data: "ParamPrepare", res: Dict, resp_data: Dict
    ) -> None:
        """
        判断依赖类型为当前执行用例的响应数据
        :param teardown_case_data: 后置请求数据
        :param res: 当前执行用例的响应数据
        :param resp_data: 当前执行用例的响应数据
        :return: 处理后的数据
        """
        try:
            ...
            if _response_dependent is not False:
                ...
                self.get_cache_name(
                    replace_key=_set_value, resp_case_data=_resp_case_data
                )
            else:
                raise JsonpathExtractionFailed(
                    f"jsonpath提取失败,替换内容: {resp_data} \n"
            ...
            raise ValueNotFoundError(
                "teardown中缺少set_cache参数，请检查用例是否正确"
            ) from exc
    ...
    def send_request_handler(
        self, data: "TearDown", resp_data: Dict, request_data: Dict
    ) -> None:
        """
        处理后置请求
        """
            ...
            if i.dependent_type == "response":
                exec(
                    self.dependent_type_response(
                        teardown_case_data=i, resp_data=resp_data
                    )
                )
            # 判断从请求内容提取数据
            elif i.dependent_type == "request":
                self.dependent_type_request(
                    teardown_case_data=i, request_data=request_data
                )
            ...
            for _data in _teardown_data:
                if _data.param_prepare is not None:
                    self.param_prepare_request_handler(
                        data=_data, resp_data=json.loads(_resp_data)
                    )
