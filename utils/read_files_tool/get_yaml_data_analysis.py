...
class CaseData:
        ...
        for key, values in dates.items():
            ...
            if key != "case_common":
                case_date = {
                    ...
                    "current_request_set_cache": self.get_current_request_set_cache(
                        values
                    ),
    ...
    def raise_value_null_error(self, data_name: Text, case_id: Text) -> Text:
        ...
        """
        detail = (
            f"用例中未找到 {data_name} 参数， 如已填写，请检查用例缩进是否存在问题"
            f"用例ID: {case_id} "
            f"用例路径: {self.file_path}"
        )
        return detail
    ...
    def raise_value_error(
        self, data_name: Text, case_id: Text, detail: [Text, list, Dict]  # type: ignore
    ) -> Text:
        """
        所有用例填写不规范的异常提示
        :param data_name: 参数名称
            ...
            if _request_type.upper() not in _types:
                raise ValueNotFoundError(
                    self.raise_value_error(
                        data_name="requestType", case_id=case_id, detail=_request_type
                    )
                )
    ...
    def get_dependence_case(self, case_id: Text, case_data: Dict) -> Dict:
        ...
        try:
            ...
            raise ValueNotFoundError(
                self.raise_value_null_error(
                    case_id=case_id, data_name="dependence_case"
                )
            ) from exc

    def get_dependence_case_data(
        self, case_id: Text, case_data: Dict
    ) -> Union[Dict, None]:
        """
        获取依赖的用例
        :return:
        """
        INFO.logger.info(f"获取依赖的用例{case_data}")
        ...
        if self.get_dependence_case(case_id=case_id, case_data=case_data):
            try:
                ...
                if _dependence_case_data is None:
                    raise ValueNotFoundError(
                        f"dependence_case_data 依赖数据中缺少依赖相关数据！"
                        f"如有填写，请检查缩进是否正确"
                        f"用例ID: {case_id}"
                        f"用例路径: {self.file_path}"
                    )
                INFO.logger.info(
                    f"获取依赖的用例数据------------------------------{_dependence_case_data}"
                )
                return _dependence_case_data
            except KeyError as exc:
                raise ValueNotFoundError(
                    self.raise_value_null_error(
                        case_id=case_id, data_name="dependence_case_data"
                    )
                ) from exc
        else:
            return None
        ...
        try:
            ...
            if _assert is None:
                raise self.raise_value_error(
                    data_name="assert", case_id=case_id, detail=_assert
                )
...
class GetTestCase:
    @staticmethod
    def case_data(case_id_list: List):
        case_data_list = []
        for i in case_id_list:
            _data = CacheHandler.get_cache(i)
            case_data_list.append(_data)
        return case_data_list


if __name__ == "__main__":
    a = CaseData("D:\PytestAutoApi\data\Login\login.yaml")
    print(a)
