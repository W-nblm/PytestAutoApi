...
class AllureFileClean:
    ...
    def get_testcases(cls) -> List:
        """获取allure报告执行用例情况"""
        files = []
        for i in get_all_files(ensure_path_sep("\\reports\\allure-results")):
    ...
    def get_case_count(cls) -> "TestMetrics":
        ...
        try:
            ...
            run_case_data = {
                k: v for k, v in data["statistic"].items() if k in keep_keys
            }
            # 判断运行用例总数大于0
            if _case_count["total"] > 0:
                # 计算用例成功率
                run_case_data["pass_rate"] = round(
                    (_case_count["passed"] + _case_count["skipped"])
                    / _case_count["total"]
                    * 100,
                    2,
                )
            ...
            run_case_data["time"] = (
                ...
                else round(_time["duration"] / 1000, 2)
            )
